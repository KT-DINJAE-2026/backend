...
| `headway_sec` | continuous | double | 직전 차량 간격(초). 첫 차량은 이전 스냅샷 이력 또는 결측, 두 번째 차량은 TOPIS `ETA2 - ETA1` — 아래 입력 방식 참고 |
...
### headway_sec 입력 방식 — TOPIS 실시간 ETA 차이
...
판정이 왜곡될 수 있어 계획값 대신 TOPIS 실시간 도착정보를 사용한다. 같은 스냅샷의 두 번째 차량은
첫 번째 차량이 직전 차량이므로 `ETA2 - ETA1`을 입력한다. 첫 번째 차량은 이전 스냅샷에서 두 번째
차량으로 관측된 뒤 첫 번째로 이동한 연결이 확인되면 앞 차량의 마지막 예상 도착시각과 현재 ETA
차이를 입력한다. 이력이 없거나 ETA 순서·차량·노선 식별이 비정상이면 임의 보정하지 않고 결측으로
처리한다. 이력은 애플리케이션 인스턴스 메모리에 `(routeId, stopOrder)` 기준으로 최대 6시간 유지한다.

학습값 역시 실제 차량 도착시각이 아니라 각 운행회차의 첫 승차 태그 시각 차이이므로 실시간 ETA
간격과 완전히 같지는 않다. 모델 A가 이 피처에 크게 의존하므로 두 분포와 예측 확률 변화를 검증해야
한다. 계획 배차간격 CSV·공급자는 향후 재학습 실험을 위해 보존하지만 현재 운영 여정에서는 사용하지
않는다.

## 여정 API 연결 상태

`demo`가 아닌 프로필의 `/api/v1/journeys/predictions`는 TOPIS 차량별 도착 예정 시각으로
8개 피처를 생성해 이 PMML 예측기를 직접 호출한다. `headway_sec`는 첫 차량의 이전 스냅샷 이력과
두 번째 차량의 실시간 ETA 차이로 구성하며, 이력이 없으면 결측이다. 학습 범위 밖 입력과 모델 미적재 상태는
