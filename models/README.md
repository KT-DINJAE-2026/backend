...
| `headway_sec` | continuous | double | 배차 간격(초). 서울시 버스노선 기본정보의 노선·운행일 유형별 계획값을 사용하며 `0`·빈값·미매핑은 결측(NaN)으로 전달한다 |
...
운영 `headway_sec`는 `1014` 같은 표시 노선번호로 계획 배차간격을 찾되, 모델의 `route_id`에는 기존
표준 노선 ID를 그대로 넣는다. 이 계획값은 학습 Parquet의 승차 태그 기반 이전 운행회차 간격과
정의가 다르므로 임시 운영 대체값이다. 최종 모델은 운영과 같은 정의로 학습 데이터를 다시 만들어
재학습하는 것이 안전하다.
