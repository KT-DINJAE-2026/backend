...
| 2 | Cloud Run 서비스 `backend` | 1 vCPU / **메모리 4GiB** + startup CPU boost(최종 PMML 적재 파싱 피크가 2GiB 초과 — OOM 실측), min 0(시연일 1) / max 2, 포트 8080, 미인증 호출 허용 | Spring Boot 백엔드 실행 |
...
메모리는 4GiB가 필요하다. 2026-08-24 배포용 최종 PMML(약 259MB)은 적재 후 상주 힙이 약 767MiB이고, 적재 중 XML 파싱 피크는 2GiB도 넘겨 2GiB 배포가 OOM으로 실패했다(실측). 기동이 51초(모델 파싱 약 41초)라 `--cpu-boost`도 함께 준다.
